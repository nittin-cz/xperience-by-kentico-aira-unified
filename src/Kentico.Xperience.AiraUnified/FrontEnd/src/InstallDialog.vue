<template>
  <div
    id="installFrameManual"
    class="c-install-modal"
    style="z-index: 100"
    hidden
  >
    <div class="c-install-modal_header">
      <img
        v-if="logoImgRelativePath !== ''"
        :src="`${baseUrl}${logoImgRelativePath}`"
        alt="Kentico"
        class="c-install-modal_logo"
      />
    ...
    <div class="c-install-modal_footer">
      <button
        id="installModalDismissButton-IOS"
        class="btn btn-outline-info text-uppercase ms-auto"
      >
        Dismiss
      </button>
    </div>
  ...
  >
    <div class="c-install-modal_header">
      <img
        ...
        class="c-install-modal_logo"
      />
      <h6 class="mb-0">Install Aira Unified</h6>
      <button
        id="installModalDismissButton-android"
        class="btn btn-outline-info fs-1 text-uppercase mx-auto"
      >
        Dismiss
      </button>
    </div>
    ...
    <div class="c-install-modal_footer">
      <button class="btn btn-outline-info text-uppercase" id="installButton">
        Install Kentico App
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    handleManualInstall() {
      if (this.isIOS()) {
        ...
        document.getElementById("installFrameManualMessage").innerHTML =
          "For full functionality, use Chrome and approve install prompt";
        document.getElementById("installFrameManual").removeAttribute("hidden");
      }
    },
    setDismissButtons() {
      document
        .querySelector("#installModalDismissButton-IOS")
        ?.addEventListener("click", () => {
          document
            .getElementById("installFrameManual")
            ?.setAttribute("hidden", true);
        });

      document
        .querySelector("#installModalDismissButton-android")
        ?.addEventListener("click", () => {
          document
            .getElementById("installFrameAndroid")
            ?.setAttribute("hidden", true);
        });
    },
    isSamsungBrowser() {
