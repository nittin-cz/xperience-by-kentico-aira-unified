<template>
  <div class="c-app_inner" v-bind:class="{ 'is-loaded': isLoaded }">
    ...
    >
      <div>
        <a
          :href="`${chatUrl}?${chatQueryParameterName}=${thread.threadId}`"
          class="c-thread"
          v-for="thread in threadsData"
        >
          <span class="d-block c-thread_icon">
            ...
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M55.7381 10.6021L64.5916 13.5533C65.8028 13.9571 65.8028 15.6703 64.5916 16.074L55.7381 19.0252C55.3414 19.1574 55.0301 19.4687 54.8978 19.8654L51.9467 28.7189C51.5429 29.9301 49.8297 29.9301 49.426 28.7189L46.4748 19.8654C46.3426 19.4687 46.0313 19.1574 45.6346 19.0252L36.7811 16.074C35.5699 15.6703 35.5699 13.9571 36.7811 13.5533L45.6346 10.6021C46.0313 10.4699 46.3426 10.1586 46.4748 9.76191L49.426 0.908415C49.8297 -0.302802 51.5429 -0.302806 51.9467 0.908411L54.8978 9.76191C55.0301 10.1586 55.3414 10.4699 55.7381 10.6021ZM54.6178 13.9631L57.1695 14.8137L54.6178 15.6642C53.1632 16.1491 52.0218 17.2905 51.5369 18.7451L50.6863 21.2968L49.8358 18.7451C49.3509 17.2905 48.2095 16.1491 46.7549 15.6642L44.2032 14.8137L46.7549 13.9631C48.2095 13.4782 49.3509 12.3368 49.8358 10.8822L50.6863 8.3305L51.5369 10.8822C52.0218 12.3368 53.1632 13.4782 54.6178 13.9631Z"
                fill="black"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M25.8924 27.1831C25.4957 27.0508 25.1844 26.7395 25.0522 26.3428L20.9956 14.1732C20.5919 12.9619 18.8786 12.9619 18.4749 14.1732L14.4183 26.3428C14.2861 26.7395 13.9748 27.0508 13.5781 27.1831L1.40841 31.2396C0.197197 31.6434 0.197194 33.3566 1.40841 33.7604L13.5781 37.8169C13.9748 37.9492 14.2861 38.2605 14.4183 38.6572L18.4749 50.8268C18.8786 52.0381 20.5919 52.0381 20.9956 50.8269L25.0522 38.6572C25.1844 38.2605 25.4957 37.9492 25.8924 37.8169L38.0621 33.7604C39.2733 33.3566 39.2733 31.6434 38.0621 31.2396L25.8924 27.1831ZM30.64 32.5L24.7721 30.544C23.3175 30.0592 22.1761 28.9177 21.6912 27.4632L19.7353 21.5952L17.7793 27.4632C17.2944 28.9177 16.153 30.0592 14.6984 30.544L8.83051 32.5L14.6984 34.456C16.153 34.9408 17.2944 36.0822 17.7793 37.5368L19.7353 43.4048L21.6912 37.5368C22.1761 36.0822 23.3175 34.9408 24.7721 34.456L30.64 32.5Z"
                fill="black"
              />
              <path
                d="M37.8968 51.5018C38.2935 51.3695 38.6048 51.0583 38.737 50.6615L40.5828 45.1242C40.9865 43.913 42.6998 43.913 43.1035 45.1242L44.9493 50.6615C45.0815 51.0583 45.3928 51.3695 45.7895 51.5018L51.3268 53.3476C52.5381 53.7513 52.5381 55.4645 51.3268 55.8683L45.7895 57.714C45.3928 57.8463 45.0815 58.1576 44.9493 58.5543L43.1035 64.0916C42.6998 65.3028 40.9865 65.3028 40.5828 64.0916L38.737 58.5543C38.6048 58.1576 38.2935 57.8463 37.8968 57.714L32.3595 55.8683C31.1483 55.4645 31.1483 53.7513 32.3595 53.3476L37.8968 51.5018Z"
                fill="black"
              />
            </svg>
          </span>
          <span class="c-thread_body">
            ...
            <span class="c-thread_content">
              {{ getThreadLastMessage(thread.lastMessage) }}
            </span>
          </span>
        </a>
...
export default {
  ...
  methods: {
    ...
    timeAgo(lastUsed) {
      const lastUsedUtc = new Date(lastUsed); // Force UTC interpretation
      const nowUtc = new Date(); // Already in UTC
      console.log(lastUsedUtc, nowUtc);
    ...
    getThreadLastMessage(message) {
      try {
        var insights = JSON.parse(message);
        return insights.query_description;
      } catch (e) {
        return message;
      }
    },
