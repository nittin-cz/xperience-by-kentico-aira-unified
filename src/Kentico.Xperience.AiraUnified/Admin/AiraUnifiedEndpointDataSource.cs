...
internal sealed class AiraUnifiedEndpointDataSource : MutableEndpointDataSource
{
    ...
    private const string ExpectedApplicationJsonOrMultipartFormData =
    ...
    public AiraUnifiedEndpointDataSource(
    ...
    private static Endpoint CreateAiraEndpointWithQueryParams(AiraUnifiedConfigurationItemInfo configurationInfo,
        ...
        => CreateEndpoint($"{configurationInfo.AiraUnifiedConfigurationItemAiraPathBase}/{subPath}", async context =>
        {
            ...
            if (!context.Request.Query.ContainsKey(actionParameterName) ||
                !int.TryParse(context.Request.Query[actionParameterName], out var val1))
            {
    ...
    private static Endpoint CreateAiraEndpointFromIFormCollectionWithRouteValue(
        ...
        => CreateEndpoint($"{configurationInfo.AiraUnifiedConfigurationItemAiraPathBase}/{subPath}", async context =>
        {
                         ...
                         StringComparison.OrdinalIgnoreCase))
            {
                ...
                var formCollection = new FormCollection(new Dictionary<string, StringValues> { { "message", body } });
    ...
    private static Endpoint CreateAiraEndpointFromBody<T>(
        ...
        => CreateEndpoint($"{configurationInfo.AiraUnifiedConfigurationItemAiraPathBase}/{subPath}", async context =>
        {
            ...
            if (context.Request.ContentType is not null &&
                string.Equals(context.Request.ContentType, ApplicationJsonIdentifier,
                    StringComparison.OrdinalIgnoreCase))
            {
                using var reader = new StreamReader(context.Request.Body);
                var body = await reader.ReadToEndAsync();
                ...
                try
                {
                    var requestObject = JsonSerializer.Deserialize<T>(body,
    ...
    private static Endpoint CreateAiraEndpointWithConditionalRedirect(
        AiraUnifiedConfigurationItemInfo configurationInfo,
        string subPath,
        string actionName,
        Func<AiraUnifiedController, Task<IActionResult>> action,
        ...
        string redirectUrl)
    {
        var path = string.Equals(subPath, string.Empty)
            ? $"{configurationInfo.AiraUnifiedConfigurationItemAiraPathBase}"
        ...
        return CreateEndpoint(path, async context =>
        {
            ...
            if (!await CheckHttps(context) ||
                await SetRedirectIfAuthorized(context, configurationInfo.AiraUnifiedConfigurationItemAiraPathBase,
                    permissionToRedirectedEndpoint, redirectUrl))
            {
                return;
            }
    ...
    private static Endpoint CreateAiraIFormCollectionEndpoint(AiraUnifiedConfigurationItemInfo configurationItemInfo,
        ...
        string? requiredPermission = null)
        => CreateEndpoint($"{configurationItemInfo.AiraUnifiedConfigurationItemAiraPathBase}/{subPath}",
            async context =>
            {
                var airaUnifiedController = await GetAiraUnifiedControllerInContext(context, actionName);
                ...
                else if (string.Equals(context.Request.ContentType, ApplicationJsonIdentifier))
                {
                    ...
                    var formCollection =
                        new FormCollection(new Dictionary<string, StringValues> { { "message", body } });
    ...
    private static async Task<AiraUnifiedController> GetAiraUnifiedControllerInContext(HttpContext context,
        string actionName)
    {
    ...
    private static async Task<bool> ValidateRequestXorSetRedirect(HttpContext context,
        AiraUnifiedConfigurationItemInfo configurationInfo, string? requiredPermission = null)
        => await CheckHttps(context)
           && (requiredPermission is null || await AuthorizeOrSetRedirectToSignIn(context,
    ...
    private static async Task<bool> AuthorizeOrSetRedirectToSignIn(HttpContext context, string airaUnifiedPathBase,
        string permission)
    {
        ...
        if (user is null || !userProvider.Get().WhereEquals(nameof(UserInfo.UserGUID), user.UserGUID).Any())
        {
            context.Response.Redirect(
                $"{signInRedirectUrl}?{AiraUnifiedConstants.SigninMissingPermissionParameterName}={permission}");
            return false;
        }

        var hasAiraViewPermission =
    ...
    private static async Task<bool> SetRedirectIfAuthorized(HttpContext context, string airaUnifiedPathBase,
        string permission, string redirectSubPath)
    {
        var adminUserManager = context.RequestServices.GetRequiredService<AdminUserManager>();
        var airaUnifiedAssetService = context.RequestServices.GetRequiredService<IAiraUnifiedAssetService>();
        var userProvider = context.RequestServices.GetRequiredService<IInfoProvider<UserInfo>>();
