using DancingGoat.Samples.CustomInsights.Components;
...
namespace Samples.DancingGoat.Strategies;
...
public sealed class UsersInsightsStrategy : InsightsStrategyBase
{
    ...
    protected override async Task<object> LoadRealDataAsync(InsightsContext context)
    {
        ...
        var newUsers =
            allUsersList.Where(u => u.LastLogin > DateTime.UtcNow.AddDays(-30))
                .ToList(); // Mock created date with last login

        var roleStats = allUsersList
            .GroupBy(u => u.Role)
            .Select(g => new UserRoleStatsModel
            {
                RoleName = g.Key, UserCount = g.Count(), Percentage = (decimal)g.Count() / allUsersList.Count * 100
            })
            .ToList();
    ...
    public override Task<object> LoadMockDataAsync(InsightsContext context) =>
        Task.FromResult<object>(new UsersInsightsDataModel
        {
            Summary =
                new UsersSummaryModel
                {
                    TotalUsers = 1250, ActiveUsers = 890, InactiveUsers = 360, NewUsersThisMonth = 45
                },
